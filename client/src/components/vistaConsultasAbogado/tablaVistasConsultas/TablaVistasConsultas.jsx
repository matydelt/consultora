...
export default function TablaVistasConsultas({
  consultas,
  usuario,
  aceptadas,
  terminoBusquedaTodas,
  terminoBusquedaAceptadas,
}) {
    ...
    <>
      <div className="my-4 mx-1">
        <table className="table table-hover table-striped">
          ...
          <tbody>
            {((terminoBusquedaTodas && terminoBusquedaTodas.length > 2) ||
            aceptadas ||
            (terminoBusquedaAceptadas && terminoBusquedaAceptadas.length > 2)
              ? consultas
              : consultasMostrar
            )
              .filter((c) => {
                if (terminoBusquedaAceptadas && aceptadas) {
                  return (
                    (c.dni.includes(terminoBusquedaAceptadas) ||
                      c.nombre
                        .toLowerCase()
                        .includes(terminoBusquedaAceptadas.toLowerCase()) ||
                      c.apellido
                        .toLowerCase()
                        .includes(terminoBusquedaAceptadas.toLowerCase())) &&
                    c.abogadoId === usuario?.abogado?.id
                  );
                } else if (terminoBusquedaTodas && !aceptadas) {
                  return (
                    c.dni.includes(terminoBusquedaTodas) ||
                    c.nombre
                      .toLowerCase()
                      .includes(terminoBusquedaTodas.toLowerCase()) ||
                    c.apellido
                      .toLowerCase()
                      .includes(terminoBusquedaTodas.toLowerCase())
                  );
                ...
                return (
                  <>
                    <tr key={consulta.createdAt} className="text-center">
                      ...
                      {consulta.abogadoId ? (
                        <td className="align-middle">
                          <span className="badge bg-warning">Aceptada</span>
                        </td>
                      ) : (
                        <td className="align-middle">
                      ...
                      <td className="align-middle w-50">{consulta.mensaje}</td>
                      ...
                      {!consulta.ticket ? (
                        ...
                        <td className="align-middle text-success">
                          <span className="badge bg-success">Abonado</span>
                        </td>
                      )}
        ...
        {((!terminoBusquedaTodas && !terminoBusquedaAceptadas && !aceptadas) ||
          consultas.length <= cantidad.current) &&
          consultas.length > cantidad.current && (
              ...
              <ul className="pagination pagination-lg">
                <div className="row mx-auto botones-paginador">
                  <li className="page-item col">
                    <button
                      disabled={pagina === 0}
                      onClick={() => setPagina(pagina - cantidad.current)}
                  ...
                  <li className="page-item col">
                    <button
                      disabled={pagina + cantidad.current >= consultas.length}
                      onClick={() => setPagina(pagina + cantidad.current)}
                      className="page-link bg-light"
                      href="#"
                      aria-label="Next"
