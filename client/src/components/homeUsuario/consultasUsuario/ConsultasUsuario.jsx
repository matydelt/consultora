  ...
  return (
    <>
      ...
      <table className="table table-hover mt-5">
        ...
        <tbody>
          {consultas
            ?.filter((c) => c.dni?.toString() === usuario.dni?.toString())
            .map((consulta) => {
              return (
                <>
                  <tr className="align-middle">
...

                    {
                      <td>
                        <input
                          ...
                          required
                          onChange={(e) => {
                            setN_Operacion(e.target.value);
                          }}
                        />
                        <button
                          disabled={consulta?.ticket?.estatus !== "pending"}
                          onClick={() => {
                            efectuarPago(consulta.ticket?.enlace, n_operacion);
                          }}
                          className={`btn btn-${
                            consulta.ticket?.estatus === "pending"
                              ? "success"
                    ...
                    <td>
                      <button
                        ...
                        data-bs-target={`#modalConsultaDetalle`}
                      >
                        Ver detalle
                      </button>
                    </td>
